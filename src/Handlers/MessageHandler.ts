...
export default class MessageHandler {
	...
	handleMessage = async (M: ISimplifiedMessage): Promise<void> => {
		...
		this.client.log(
			...
			if (!command)
			return void M.reply( await request.buffer(`https://c.tenor.com/hrMpWeOG6XAAAAPo/elbicho-vicotrevs.mp4`),
                    MessageType.video,
                    Mimetype.gif,
                    undefined,
                    `No such command, Use *${this.client.config.prefix}help*`,
                    undefined
                )
		...
		if (command.config?.adminOnly && !M.sender.isAdmin)
			return void M.reply(
				`This command is only meant for the group admins`
			);
		try {
